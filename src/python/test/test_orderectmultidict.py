...
from src.python.orderedmultidict import omdict
...
class TestOmdict(unittest.TestCase):
    ...
    def test_init(self):
        for init in self.inits:
            omd = omdict(init)
            self.assertEquals(omd.allitems(), init.items())

            omd1 = omdict(init)
            omd2 = omdict(omd1)
            self.assertEquals(omd1.allitems(), omd2.allitems())

    def test_load(self):
        omd = omdict()
        for init in self.inits:
            self.assertEquals(omd.load(init), omd)
            self.assertEquals(omd.allitems(), init.items())
    ...
    def test_copy(self):
        for init in self.inits:
            omd = omdict(init)
            copy = omd.copy()
            self.assertFalse(omd is copy)
            self.assertEquals(omd, copy)

    def test_clear(self):
        for init in self.inits:
            omd = omdict(init)
            omd.clear()
            self.assertEquals(len(omd), 0)
            self.assertEquals(omd.items(), [])

    def test_fromkeys(self):
        for init in self.inits:
            keys = [key for key, value in init.items()]
            allitems = omdict.fromkeys(keys, _unique).allitems()
            self.assertEquals(allitems, zip(keys, repeat(_unique)))

    def test_has_key(self):
        for init in self.inits:
            omd = omdict(init)
            for key, value in init.items():
                self.assertTrue(omd.has_key(key))
